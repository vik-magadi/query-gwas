...
def plot_variant(connection,var):
    ...
    #creates a bar graph of the p-values of a variant
...
def plot_gene(connection, gene):
    ...
    p = []
    #creates multiple bar graphs, one for each variant on a certain gene
    for var in var_list:
        v = var[0]
        cur = con.cursor()
...
def plot_trait(connection, trait):
    ...
    name=positions['Name']))
    #information that is displayed when the user hovers over a point on the graph
    TOOLTIPS = [
    ("Variant", "@name"),
    ("P-value", "@pvals")]
